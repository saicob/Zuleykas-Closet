...
function mostrarModalProducto(prod) {
    ...
    if (prod.talla && prod.tallas && Array.isArray(prod.tallas)) {
        ...
        if (
            ...
            tallaObj.imagen &&
            tallaObj.imagen !== "" &&
            tallaObj.imagen !== null &&
            tallaObj.imagen !== undefined
        ) {
    ...
    if (!imagenMostrar || imagenMostrar === "/placeholder.svg?height=50&width=50") {
        ...
        noImg.style.cssText =
            "width: 260px; height: 200px; display: flex; align-items: center; justify-content: center; color: #999; background: #f5f5f5; border-radius: 12px; margin: 0 auto 18px; font-size: 20px; font-weight: 500;"
        ...
        img.alt = prod.nombre
        img.style.cssText =
            "width: 260px; height: 200px; object-fit: cover; border-radius: 12px; background: #f5f5f5; display: block; margin: 0 auto 18px; box-shadow: 0 2px 8px rgba(0,0,0,0.07)"
    ...
    h3.textContent = prod.nombre
    h3.style.cssText =
        'font-size: 2rem; font-family: "Montserrat", Arial, sans-serif; font-weight: 700; margin-bottom: 12px; color: #222; letter-spacing: 0.5px;'
    card.appendChild(h3)
    ...
    if (prod.descripcion) {
        ...
        desc.style.cssText =
            "font-size: 1.08rem; color: #333; margin-bottom: 18px; max-width: 100%; white-space: normal; overflow-wrap: break-word; text-align: center;"
    ...
    if (tallas.length > 1 || (["Ropa", "Accesorio"].includes(prod.categoria) && tallas.length > 1)) {
        ...
        tallaGroup.style.cssText =
            "margin: 12px 0 10px 0; display: flex; align-items: center; justify-content: center; gap: 10px;"
        ...
        cantidadGroup.style.cssText =
            "display: flex; align-items: center; justify-content: center; gap: 12px; margin: 22px 0 10px 0;"
        ...
        btnCancelar.style.cssText =
            "background: #f5f5f5; color: #444; border: none; border-radius: 10px; padding: 10px 28px; font-size: 1.08rem; font-weight: 500; cursor: pointer; transition: background 0.18s;"
        ...
        btnAgregar.style.cssText =
            "background: #f8a8b9; color: #fff; border: none; border-radius: 10px; padding: 10px 28px; font-size: 1.08rem; font-weight: 600; cursor: pointer; box-shadow: 0 2px 8px rgba(248,168,185,0.08); transition: background 0.18s;"
        ...
        if (
            prod.talla &&
            (!prod.tallas ||
                prod.tallas.length <= 1 ||
                (prod.tallas.length > 1 && prod.tallas.every((t) => t.talla === prod.talla)))
        ) {
...
function mostrarToast(mensaje) {
    ...
    if (!toast) {
        ...
        toast.style.cssText =
            "position:fixed;bottom:32px;left:50%;transform:translateX(-50%);background:#f8a8b9;color:#fff;padding:16px 32px;border-radius:12px;font-size:1.1rem;box-shadow:0 2px 12px rgba(0,0,0,0.10);z-index:10000;opacity:0;pointer-events:none;transition:opacity 0.3s;"
...
// Hacer el cache disponible globalmente para el carrito
window.productosCache = productosCache
window.mostrarModalProducto = mostrarModalProducto

// Actualizar el cache global cada vez que se carga
const originalCargarCatalogo = cargarCatalogo
cargarCatalogo = async () => {
    await originalCargarCatalogo()
    window.productosCache = productosCache
}
